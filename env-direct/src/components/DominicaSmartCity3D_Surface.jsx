...
const DominicaSmartCity3D = () => {
  ...
  // Create enhanced DEM surface for solid terrain (sorted row-major for better surface quality)
  const createIslandSurface = () => {
    const surface = [];
  ...
  // Create coastline at sea level for 3D island outline
  const createCoastline = () => {
    const coastline = [];
  ...
  useEffect(() => {
    const loadGeoJsonData = async () => {
      try {
        setLoading(true);
        console.log('Loading Dominica building data...');
        
        const response = await fetch('/dominica-buildings.geojson');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    if (cityData.buildings.length === 0) {
      console.warn('No buildings data to display');
      return;
    }
    ...
    if (viewMode === 'enhanced3D') {
      // Enhanced 3D Island View with surface3D terrain
      const islandSurface = createIslandSurface();
      const coastline = createCoastline();
      ...
      config = {
        backgroundColor: '#071529', // Deep ocean blue like your HTML example
        animation: false,
        title: {
          text: '🏝️ Dominica - Realistic 3D Terrain + Buildings',
          subtext: 'Surface3D Terrain • Coastline • Building Database',
          textStyle: { 
            color: '#cfe3ff', 
            fontSize: 16,
            fontWeight: 'bold'
        ...
        visualMap: {
          ...
          inRange: { 
            color: ['#2b83ba','#abdda4','#ffffbf','#fdae61','#d7191c'] // Your color scheme
          },
          top: '10%',
          right: '5%'
        },
        ...
        series: [
          ...
          // 3. Buildings as 3D bars
          {
            name: 'Buildings',
            type: 'bar3D',
            data: buildingBars,
            barSize: 0.5, // Smaller bars for better visibility
            minHeight: 0.5,
            shading: 'lambert',
            itemStyle: { 
              opacity: 0.96,
        ...
        {viewMode === 'enhanced3D' && (
          <div className="mt-3 pt-3 border-t border-gray-600">
            ...
            <div className="space-y-1 text-xs text-gray-300">
              ...
              <div>• Coastline at sea level</div>
              <div>• Morne Diablotins peak</div>
              <div>• Realistic elevation model</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="mt-3 pt-2 border-t border-gray-600">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-gray-300 text-xs">Coastline (Sea Level)</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 bg-gradient-to-r from-blue-600 to-red-600 rounded"></div>
                <span className="text-gray-300 text-xs">Surface3D Terrain</span>
              </div>
            </div>
          </div>
        ) : (
